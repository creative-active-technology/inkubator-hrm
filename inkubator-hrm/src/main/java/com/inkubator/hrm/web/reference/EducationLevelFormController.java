...
public class EducationLevelFormController extends BaseController {
	...
	public void doSave(){
		...
		try {
			...
			MessagesResourceUtil.setMessages(FacesMessage.SEVERITY_ERROR, "global.error", "educationlevel.error_duplicate_name", FacesUtil.getSessionAttribute(HRMConstant.BAHASA_ACTIVE).toString());
			doClear();
		}
		catch(Exception ex){
			LOGGER.error("Error", ex);
