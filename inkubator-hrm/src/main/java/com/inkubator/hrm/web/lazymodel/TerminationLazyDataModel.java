    ...
    public List<Termination> load(int first, int pageSize, String sortField, SortOrder sortOrder, Map<String, String> filters) {
        ...
        if (sortField != null) {
            if (sortOrder == SortOrder.ASCENDING) {
                try {
                    terminationList = service.getAllDataWithAllRelation(searchParameter, first, pageSize, Order.asc(sortField));
                ...
                try {
                    terminationList = service.getAllDataWithAllRelation(searchParameter, first, pageSize, Order.desc(sortField));
                    jumlahData = Integer.parseInt(String.valueOf(service.getTotalTerminateByParam(searchParameter)));
                } catch (Exception ex) {
                    LOGGER.error("Error", ex);
                }
            ...
            try {
                terminationList = service.getAllDataWithAllRelation(searchParameter, first, pageSize, Order.desc("code"));
                jumlahData = Integer.parseInt(String.valueOf(service.getTotalTerminateByParam(searchParameter)));
            } catch (Exception ex) {
                LOGGER.error("Error", ex);
            }
