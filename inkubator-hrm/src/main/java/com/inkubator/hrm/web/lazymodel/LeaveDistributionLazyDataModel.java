    ...
    public List<LeaveDistribution> load(int first, int pageSize, String sortField, SortOrder sortOrder, Map<String, String> filters) {
        ...
        if (sortField != null) {
            if (sortOrder == SortOrder.ASCENDING) {
                try {
                    leaveDistributionList = service.getByParamWithDetail(searchParameter, first, pageSize, Order.asc(sortField));
                ...
                try {
                    leaveDistributionList = service.getByParamWithDetail(searchParameter, first, pageSize, Order.desc(sortField));
                    jumlahData = Integer.parseInt(String.valueOf(service.getTotalLeaveDistributionByParam(searchParameter)));
                } catch (Exception ex) {
                    LOGGER.error("Error", ex);
                }
            ...
            try {
                leaveDistributionList = service.getByParamWithDetail(searchParameter, first, pageSize, Order.desc("empData"));
                jumlahData = Integer.parseInt(String.valueOf(service.getTotalLeaveDistributionByParam(searchParameter)));
            } catch (Exception ex) {
                LOGGER.error("Error", ex);
            }
