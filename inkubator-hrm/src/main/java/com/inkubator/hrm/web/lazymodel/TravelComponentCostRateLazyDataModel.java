    ...
    public List<TravelComponentCostRate> load(int first, int pageSize, String sortField, SortOrder sortOrder, Map<String, String> filters) {
        ...
        if (sortField != null) {
            if (sortOrder == SortOrder.ASCENDING) {
                try {
                    travelComponentCostRateList = service.getAllDataWithAllRelation(parameter, first, pageSize, Order.asc(sortField));
                ...
                try {
                    travelComponentCostRateList = service.getAllDataWithAllRelation(parameter, first, pageSize, Order.desc(sortField));
                    jumlahData = Integer.parseInt(String.valueOf(service.getTotalTravelComponentRateByParam(parameter)));
                } catch (Exception ex) {
                    LOGGER.error("Error", ex);
                }
