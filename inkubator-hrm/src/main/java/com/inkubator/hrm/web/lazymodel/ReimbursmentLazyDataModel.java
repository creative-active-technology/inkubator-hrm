    ...
    public List<Reimbursment> load(int first, int pageSize, String sortField, SortOrder sortOrder, Map<String, String> filters) {
        ...
        if (sortField != null) {
            if (sortOrder == SortOrder.ASCENDING) {
                try {
                    reimbursmentList = service.getAllDataWithDetail(parameter, first, pageSize, Order.asc(sortField));
                ...
                try {
                    reimbursmentList = service.getAllDataWithDetail(parameter, first, pageSize, Order.desc(sortField));
                    jumlahData = Integer.parseInt(String.valueOf(service.getTotalReimbursmentByParam(parameter)));
                } catch (Exception ex) {
                    LOGGER.error("Error", ex);
                }
            ...
            try {
                reimbursmentList = service.getAllDataWithDetail(parameter, first, pageSize, Order.desc("code"));
                jumlahData = Integer.parseInt(String.valueOf(service.getTotalReimbursmentByParam(parameter)));
            } catch (Exception ex) {
                LOGGER.error("Error", ex);
            }
