...
import com.inkubator.hrm.service.CompanyPolicyCronService;

/**
 *
 * @author rizkykojek
 */
public class CompanyPolicyCronServiceImpl extends IServiceImpl implements CompanyPolicyCronService {
	...
	/**
     * <p>Method untuk mem-broadcast email dari kebijakan perusahaan. 
     * Akan dilakukan pengecekan berdasarkan waktu efektif berlakunya, dan opsi pengirimannya ada yang Mingguan/Bulanan/Triwulan.
     * 
     * </p>
     *
     * <pre>
		...
		for(CompanyPolicy companyPolicy : companyPolicies){
			if(companyPolicy.getIsBroadcast() && current.after(companyPolicy.getEffectiveDate())){
				if(StringUtils.equals(companyPolicy.getRepeatOn(), HRMConstant.COMP_POLICY_REPEAT_ON_WEEKLY)){
					...
					/** jika hari-nya sama (rabu == rabu) maka execute proses sending email */
					...
					/** di cek jika ternyata effectiveDay nya 31 Maret, 
					 *  maka pada akhir bulan april yaitu 30 April harus dieksekusi process sending emailnya */
					...
					if(totalDiff % 3 == 0){
						...
						/** di cek jika ternyata effectiveDay nya 31 Maret, 
						 *  maka pada akhir bulan april yaitu 30 April harus di execute process sending emailnya */
						if((currentDay == effectiveDay) || ((effectiveDay > maximumDay) && (currentDay == maximumDay))){
							this.sendingEmailProcess(companyPolicy);
						}
					}
	...
	/**
     * <p>Method untuk mengirim broadcast email ke seluruh karyawan berdasarkan departemen dan golongan jabatan tertentu.
     * Hanya employee/karyawan yang aktif dan yang memiliki alamat email yang valid yang akan di proses.
     * 
     * </p>
     *
     * <pre>
